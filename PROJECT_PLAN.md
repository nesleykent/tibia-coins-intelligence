...
**Last reviewed:** 2026-08-03 (GP-emission rebuild: AN-10, AN-11 verified; AN-12, AN-13 opened)
...
| AN-13 | P1 | Extend the monetary panel to the 3.5-year archive | The GP series covers 2025-12-03 onward while the activity panel reaches 2022-08-23, so long-horizon tests still stand on kill counts alone | `34b_emission_history.py` streams `tibia-kill-stats-from-2022-08-23-to-2025-12-04` world by world, the union passes the 50% per-world coverage floor, and models plus artifacts are rebuilt on the long panel | `NEXT` — the repository's own pre-2025 kill data is `kill_stats_daily.csv` (aggregate) and `kill_stats_mix.csv` (top-40 creature shares, 28.9% of deaths on average), which is far below the 80% coverage gate and cannot carry a per-creature GP reconstruction. The per-creature history exists only in the remote archive. `34b` was repaired after the first attempt was withdrawn for writing structural zeros and now loads the catalogue explicitly and raises below the coverage floor; the cost is the download, not the method |
