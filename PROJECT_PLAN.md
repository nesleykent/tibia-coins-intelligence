...
| SITE-02 | P1 | Build an Executive Investment Summary decision surface | The navigation exposed analytical tools but not one place answering buy/sell/hold/avoid, fair-value context, confidence, horizon, and risks | World selection produces an as-of-dated decision card; separates observation, forecast, and action; includes price/unit, horizon, range, confidence, costs, risks, invalidation, model used, freshness, and “insufficient evidence” state | `VERIFIED` — Claude, 2026-08-03; a `Decision` view renders `decision_policy.{csv,json}` for all 61 worlds. The card separates **Observed** (price and deviation), **Fitted change** (labelled as losing to a random walk, so it is never read as a forecast to trade), **Cost to act** (round-trip Market fee) and **Model used**, above the written reason for the call and an expandable “what would make this call wrong”. Abstain renders the insufficient-evidence state and correctly omits the invalidation list, there being no call to invalidate. Nothing is recomputed in the browser, so the card cannot drift from the policy. Verified in-browser across the Buy TC, Hold and Abstain states |
| AN-01 | P1 | Define the decision and recommendation policy | A polished site must not turn raw forecasts into unsupported investment calls | Versioned rules map validated signals to Buy/Sell/Hold/Avoid/Investigate; transaction costs, liquidity, world eligibility, uncertainty, horizon, and conflicting signals are explicit; backtest and holdout evidence attached; policy can abstain | `VERIFIED` — Claude, 2026-08-03; `scripts/53_decision_policy.py` v1.0.0 emits `decision_policy.{csv,json}` — a call, a confidence and a written reason per world, plus the rule set, the evidence behind each threshold and the limitations. Only relative deviation is admitted, because only it survives a holdout; the absolute price forecast is refused as a direction because it loses to a random walk at all four horizons, and gold emission is refused outright. Trades fire at the cutoff the holdout was actually earned at (9.89% for the 7-day horizon), not at the 4% level where closure is merely significant — an earlier draft quoted the +1.73% holdout return to justify 4% trades, attaching evidence to a rule that never generated it, and correcting that took the trade calls from 29 of 61 worlds to 1. The 30- and 91-day horizons are refused for resting on 7 and 1 effective episodes despite t = 5.8 and 5.4. Current output: 56 Hold, 4 Abstain, 1 Buy TC |
