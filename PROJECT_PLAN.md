...
**Last reviewed:** 2026-08-03 (GP-emission rebuild: AN-10, AN-11, AN-13 verified; AN-12, AN-14 open)
...
| AN-12 | P2 | Re-export the standalone Gold Emission report | `reports/gold_emission_report.html` is an external "portable artifact" export with no generator in the repo; it still states 21,412 world-days against the rebuilt 21,691 and pre-dates the corrected GP figures | The HTML is regenerated from `gold_emission_report_artifact.json` and `46_verify_artifacts.py` reports no coverage gap | `NEXT` — the artifact JSON is current and already carries the direct-test verdict; only the HTML export is stale. It cannot be rebuilt from this repository, and it must not be hand-edited, because that is the PDF/site drift `AGENTS.md` forbids |
| AN-14 | P2 | Publish the conservative-NPC and category-realisation scenarios on the site | Both series now exist in `gold_emission_daily.csv`, are carried through the models and appear in the sensitivity grid, but `scripts/emission_view.py` still offers only Direct GP and Potential GP, so a reader cannot see what the buyer-access assumption or the per-category realisation assumption is worth | Gold Emission exposes `potential_total_gp_conservative` and `realized_estimate_gp_category` as selectable series with their governing caveats, and the Creature GP page shows the conservative per-kill column; hub and artifact verifiers pass | `NEXT` — deliberately deferred rather than half-built: the view is one shared component behind the hub and both standalone pages, and adding a series touches the chart, the checkboxes, the sortable table and the drill-down together. `AGENTS.md` treats analysis that has not reached every publication surface as incomplete, so this is a real coverage gap, not polish |
