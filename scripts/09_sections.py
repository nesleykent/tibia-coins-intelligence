...
para(tag("lim") + "<b>The monetary half of this test is not done, and an earlier attempt at "
     "it was withdrawn.</b> Extending the GP series over the same archive requires running the "
     "emission reconstruction across it, and the run that produced the first version of this "
     "paragraph called that reconstruction without loading the creature catalogue it depends "
     "on. Every historical row came back at zero emission, so three quarters of the series was "
     "structurally empty and the cells estimated on it measured nothing. They are removed "
     "rather than reported with a caveat. Until the reconstruction is rerun correctly, the "
     "long-horizon result stands on kill counts alone.")
