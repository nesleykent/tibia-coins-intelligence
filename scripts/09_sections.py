...
_OBS_PER_WORLD = FD['panel']['median_obs_per_world']
_FUND_YEARS = ((pd.Timestamp(FD['panel']['end']) - pd.Timestamp(FD['panel']['start']))
               .days / 365.25)
...
LHM = pd.DataFrame(LHP.get("monetary_long", []))
...
para(tag("obs") + f"The kill-statistics archive aggregates to {KS_ROWS:,} world-days over "
     ...
     f"fitted on. "
     f"The span is {_FUND_YEARS:.1f} years, which is "
     f"{'enough for several passes through the seasonal year' if _FUND_YEARS >= 2 else 'a single pass through the seasonal year'}"
     f", so the calendar features in Section 6.6 rest on "
     f"{int(max(1, round(_FUND_YEARS)))} observed cycles rather than on one.")
...
h3("6.6.1 Player activity leads the market; gold production does not")
...
para(tag("judg") + f"<b>The survivors are not borderline, and they are all measures of how many "
     f"people are playing and trading rather than of how much gold is produced.</b> Weekly "
     f"turnover leads at p below 1e-12 and deaths per player at 1e-08, margins no correction "
     f"applied here disturbs. Monsters killed survives too, but it enters as a headcount of "
     f"activity: Sections 6.6.14 and 6.6.15 convert the same hunting into gold and find no "
     f"channel from the gold itself, so what the kill series carries is the player swinging "
     f"the sword, not the coin that falls.")
para(tag("econ") + "<b>That split is the section's finding, and it points the same way as the "
     "rest of the study.</b> A production economy - gold generated by hunting, coins bought "
     "with gold - would show the monetary aggregate leading the price. What leads it instead is "
     "participation. Demand for coins is demand from players who are online, dying, and "
     "trading; the supply of gold behind them moves the price through no measurable path of "
     "its own. Market design and behaviour dominate monetary conditions here, and this is the "
     "test that shows it directly rather than by elimination.")
para(tag("lim") + f"<b>Predictive precedence in sample is not forecasting skill, and the next "
     f"section is where that is settled.</b> These tests ask whether past values of a series "
     f"reduce the residual variance of the return within the estimation window. They are run "
     f"on a daily aggregate spanning three and a half years, so a small and stable lead becomes "
     f"detectable through sample size alone. Section 6.6.2 puts the same series through "
     f"walk-forward estimation against a random walk, where the lead has to pay for itself out "
     f"of sample, and reports what survives that instead.")

h3("6.6.2 The level is forecastable only where it barely matters")
...
para(tag("stat") + f"On the cross-world relative return the answer changes, consistent with "
     ...
     f"Benjamini-Hochberg correction at 5%, win more folds than they lose, and beat a forecast "
     f"of no change out of sample. "
     f"{int(FDM.query('beats_rw == True and target == \'rel\'').shape[0])} of those "
     f"{FD['n_beating_rw_after_bh']} are on the relative target and "
     f"{int(FDM.query('beats_rw == True and target == \'ret\'').shape[0])} on the level, and "
     f"the level survives only at the shortest horizons: at thirty days nothing beats the random "
     f"walk on the level at all, while the relative target still does.")
table([["Horizon", "Model", "Target", "Out-of-sample R²", "Directional accuracy", "DM z", "Folds better"]] +
      [[f"{int(r.horizon)}d", r.model, "relative" if r.target == "rel" else "level",
        f"{r.r2_oos:+.3f}", f"{r.dir_acc:.1%}",
        f"{r.dm_z:+.2f}", f"{int(r.folds_better)} of {int(r.folds)}"]
       for _, r in FDM.query("beats_rw == True").sort_values(["target", "horizon"]).iterrows()],
      [16 * mm, 30 * mm, 20 * mm, 26 * mm, 30 * mm, 16 * mm, AVAIL - 138 * mm],
      align=[None, None, None, "R", "R", "R", "C"],
      caption="Table 6.15 - Models beating a random walk after correcting across all "
              "model-horizon comparisons, winning a majority of walk-forward folds, and "
              "improving on a forecast of no change. Diebold-Mariano z is a signed Stouffer "
              "combination across folds.")
_BEST = FDM.query("beats_rw == True").nlargest(1, "r2_oos").iloc[0]
_SWEEP = int(FDM.query("beats_rw == True and folds_better == folds").shape[0])
para(tag("stat") + f"<b>The strongest cell is a random forest on the relative target at "
     f"{int(_BEST.horizon)} days</b> - out-of-sample R-squared {_BEST.r2_oos:.3f}, directional "
     f"accuracy {_BEST.dir_acc:.1%} - and {_SWEEP} of the {FD['n_beating_rw_after_bh']} "
     f"surviving cells win every fold rather than merely most of them. On three and a half "
     f"years the edge is steadier than the earlier eight-month window could show, and it is "
     f"still small: the largest variance reduction anywhere in the table is under seven "
     f"percent, and directional accuracy never reaches two thirds.")
para(tag("judg") + f"<b>What separates the two targets is horizon, and that is the shape the "
     f"rest of the study predicts.</b> The level survives only at one and seven days, where a "
     f"forecast is mostly a statement about the last observation; by thirty days it is gone "
     f"entirely. Relative position survives at every horizon tested, including thirty. A "
     f"cost-bounded rate against a fixed anchor should behave exactly this way - the level is "
     f"pinned by the anchor and wanders, while the spread between worlds is pulled back by "
     f"arbitrage that pays only once it is wide enough.")
...
para(tag("stat") + f"Redone on the deviation itself, over the full price history rather than "
     f"the fundamentals window, and split at the estimated band, the picture is "
...
para(tag("judg") + f"<b>The production tests in this chapter ran at one, seven and thirty days "
     f"on eight months of data - the window the archive then reached - and that combination "
     f"can only find a fast channel.</b> A "
...
para(tag("lim") + f"<b>The monetary series is untested at these horizons rather than "
     f"rejected at them, and the distinction is the whole of it.</b> Gold emission in GP is "
     f"reconstructed for {LHP.get('monetary_n', 0):,} world-days spanning "
     f"{LHP.get('monetary_span_years', 0):.2f} years, while the horizons this section tests run "
     f"to {max(LHP.get('horizons_tested', [365]))} days. A calendar that short contains no "
     f"independent window of that length, so of {len(LHM)} cells none is estimable at the "
     f"three-window minimum this section requires, and the long-horizon result stands on kill "
     f"counts alone.")
para(tag("stat") + f"<b>Those cells are also a clean demonstration of why the per-date "
     f"correction is not a formality.</b> Pooled across overlapping windows, "
     f"{LHP.get('monetary_n_significant_pooled', 0)} of the {len(LHM)} monetary cells clear the "
     f"five percent level, two of them at t above seven. Not one of them is a finding. The "
     f"same overlap that manufactures those t-statistics is what leaves the calendar with no "
     f"independent window to estimate them on, and a reader shown only the pooled column would "
     f"see a strong monetary channel in a series too short to contain one.")
para(tag("lim") + f"<b>What the extended sample does not fix.</b> Three and a half years "
     f"is four seasonal cycles, not forty, and the longest horizon tested rests on "
     f"{int(LHH.independent_windows.min())} independent windows at worst. A channel operating "
     f"over several years - a slow revaluation of gold itself - would still be invisible here, "
     f"and this section closes the horizons it could test rather than every horizon that "
     f"exists.")
story.append(PageBreak())
...
para(tag("lim") + "<b>What this section cannot support.</b> Eight months is one seasonal cycle, "
     ...
     f"fusion transformers, N-BEATS - were not fitted: a median of {_OBS_PER_WORLD:,} daily "
     f"observations per world is two "
...
para(tag("judg") + "<b>The verdict of Section 7.5 is unchanged, and now rests on more.</b> The "
     ...
     "observed. The economy is now partly observed, measures of participation do lead the "
     "market in sample while gold production leads nothing, and the "
     "level is beaten out of sample only at one and seven days and by a few percent of "
     "variance, never at thirty. What the fundamentals do buy is a modest, repeatable edge on "
