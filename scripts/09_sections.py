...
para(tag("stat") + f"<b>And now on the money, over the same span.</b> Reconstructing GP "
     f"emission across the extended archive gives {LHP['monetary_n']:,} world-days over "
     f"{LHP['monetary_span_years']:.1f} years, so the cumulative specifications run on the "
     f"variable the economic question is actually about rather than on a proxy for it. "
     f"{LHP['monetary_n_significant_pooled']} of {len(LHM)} cells are significant.")
para(tag("econ") + f"<b>Two things settle it, and neither is the p-value.</b> The largest "
     f"absolute elasticity anywhere is {LHP['monetary_max_abs_elasticity']:.5f}: doubling a "
     f"world's cumulative gold emission would move its coin price by "
     f"{pc(LHP['monetary_max_abs_elasticity'] * 100, 2)}, against an effective spread of "
     f"{pc(FN['roll']['median_roll_spread_pct'])} and annual volatility of "
     f"{pc(D['ret_sd_ann_pct'], 0)}. An effect two orders of magnitude below the cost of "
     f"trading on it is not a channel a reader can use, whatever its significance.")
para(tag("lim") + f"<b>The significance itself should not be read as strength.</b> The largest "
     f"per-date statistics here reach absurd magnitudes on five independent windows, and the "
     f"reason is mechanical: cumulative emission is nearly identical across worlds on any given "
     f"date, so the cross-sectional slope has almost no sampling variance and its t-statistic "
     f"inflates without the estimate meaning more. The elasticity is the number to read.")
para(tag("judg") + f"<b>The sign does not support the inflation story either.</b> The price is "
     f"gold per coin, so more gold in circulation should require <i>more</i> gold per coin - a "
     f"positive coefficient. Across the {len(LHM)} monetary cells the sign splits roughly "
     f"evenly, {int((LHM.coef < 0).sum())} negative and {int((LHM.coef > 0).sum())} positive, "
     f"which is what an absent relationship looks like rather than a weak one.")
