...
comparison_page(
    ...
    [
        ...
        ("<b>The average price across worlds</b> as the market's return.",
         f"A basket that changes composition. Coverage grows from one world per date to 85, so "
         f"a naive mean mixes price change with new worlds entering. Chain-linking removes it: "
         f"{pc(IX['total_pct'], 1, True)}, not +44%.",
         "5x", "Section 4.2"),
...
hero(pc(IX["total_pct"], 1, True),
     ...
     "A naive average of the worlds in the archive shows +44%. The difference is not price: "
     "it is the basket changing as coverage grows from one world per date to 85. The "
     "chain-linked index removes it.",
     mark="index")
h2sec('4.2', 'Market indices', 'The market appreciated 8.6%, not the 44% a naive average implies')
...
h3("6.6.1 Two fundamentals do lead the market")
para(tag("stat") + f"Granger tests on the market aggregate, searching lags of 1 to 14 days and "
     f"controlling the false-discovery rate across the {FD['granger']['n_tested']} series "
     f"tested, leave {FD['granger']['n_survive_bh_5pct']} survivors - both at a one-day lag.")
table([["Fundamental", "Best lag", "p", "Survives FDR"]] +
      [[r.feature.replace("log_", "").replace("_", " "), f"{int(r.best_lag)}d",
        pval(r.best_p), "yes" if r.survives_bh else "no"]
       for _, r in FDL.head(6).iterrows()],
      [62 * mm, 22 * mm, 24 * mm, AVAIL - 108 * mm], align=[None, "R", "R", "C"],
      caption="Table 6.14 - Granger tests of fundamentals against the market return, best lag "
              "per series, Benjamini-Hochberg at 5% across all series tested.")
para(tag("econ") + "That monsters killed and players online lead the market return by a day is "
     "what an economy with a production lag looks like: gold is generated by hunting, and the "
     "coin is bought with gold. It is a genuine leading relationship. Whether it is a "
     "<i>usable</i> one is a different question, and the rest of this section answers it "
     "separately.")
...
para(tag("judg") + "<b>The verdict of Section 7.5 is unchanged, and now rests on more.</b> The "
     ...
     "observed. The economy is now partly observed, it does lead the market by a day, and the "
