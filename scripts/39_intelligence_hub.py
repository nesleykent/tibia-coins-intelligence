    ...
    <main class="main">
      ...
      <section id="view-worlds" class="view" hidden>
        ...
        <div class="world-layout">
          <article class="panel chart-panel"><div class="panel-heading"><div><h2 id="worldChartTitle">Compare TC prices</h2><p id="worldChartDescription">The daily average paid in GP for one Tibia Coin.</p></div><div><div id="worldLegend" class="legend"></div><div id="worldChartMode" class="segmented" aria-label="Choose what price to compare"><button class="segment active" data-mode="mid" title="Mid — the average of the buy and sell sides">Mid</button><button class="segment" data-mode="buy" title="Daily average Piece Price on the Sell Offers">Sell Offers</button><button class="segment" data-mode="sell" title="Daily average Piece Price on the Buy Offers">Buy Offers</button><button class="segment" data-mode="return" title="Return — price change since the first day in the period">Return (%)</button></div></div></div><div class="chart-wrap"><svg id="worldChart" class="chart" role="img" aria-labelledby="worldChartTitle worldChartDescription"></svg><div id="worldTooltip" class="chart-tooltip"></div></div></article>
...
function drawTwoSeries(svg,tooltip,rows,labels,mode){
  ...
  const cross=addSvg(svg,"line",{x1:m.left,x2:m.left,y1:m.top,y2:m.top+ih,stroke:"#34405a",opacity:0});
  const measureLabel={mid:"the midpoint, the daily average price",buy:"the price paid to buy TC",sell:"the price received for selling TC",return:"return, the price change since the start"}[mode];
