...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .game-context{padding:12px 16px;margin-bottom:16px}
    .game-context .claim-details{margin-top:0;border-top:0;padding-top:0}
    .game-context p{margin:0 0 10px;color:var(--muted);font-size:13px;line-height:1.6;max-width:84ch}
    .game-context p:last-child{margin-bottom:0}
    .game-context strong{color:#34405a}
    ...
    .fact span{color:var(--muted);display:flex;flex-direction:column;gap:2px;min-width:0}
    .fact span b{color:#34405a;font-weight:750}
    .fact span small{font-size:10.5px;line-height:1.35;color:var(--muted);white-space:normal}
    .fact strong{text-align:right;white-space:nowrap}
  ...
  <div class="workspace">
    ...
    <main class="main">
      <section id="view-overview" class="view">
        ...
        <article class="panel game-context">
          <details class="claim-details"><summary>New to Tibia? What these markets are</summary>
            <p>Tibia is an online game played on many separate servers called <strong>worlds</strong>; the count tracked here is shown in the Worlds metric on this page. Each world is a separate local market: characters, gold and offers live inside one world, and moving between them requires a paid character world transfer, so capital mobility is costly and restricted rather than free. That is why the same Tibia Coin can trade at different prices in different worlds without an instant arbitrage closing the gap.</p>
            <p><strong>Gold pieces (GP)</strong> are the domestic currency of each world, produced endogenously by players hunting monsters and selling loot to NPCs — this analysis calls that gold emission. <strong>Tibia Coins (TC)</strong> are the premium currency: primary supply is issued by the developer, CipSoft, for real money, and holders may then resell TC to other players for GP. All prices here are GP per 1 TC.</p>
            <p>That resale happens on the in-game <strong>Market</strong>, an order-driven venue with resting buy and sell offers, visible depth, and a Market fee on completed trades. Offers rest in the book of one world only, so each world has its own best bid, best ask, spread and depth. Worlds also differ by PvP type, BattlEye protection cohort and geographic region, which is why the models group them along those dimensions.</p>
          </details>
        </article>
      ...
      <section id="view-worlds" class="view" hidden>
        <div class="view-header"><div><h1>Worlds</h1><p class="view-intro">See how much GP you pay to buy TC, how much you receive when selling TC, and how prices changed in each world. The professional order-book terms stay on the labels, each with its plain meaning next to it.</p></div>
        ...
        <article class="panel" style="margin-top:16px"><div class="panel-heading"><div><h2>Executable prices now</h2><p>What you can actually pay or receive in each world's Market right now.</p><details class="claim-details"><summary>How these numbers are calculated</summary><p>Each world runs its own order-driven Market with resting buy and sell offers. <strong>Best ask</strong> is the cheapest standing sell offer and <strong>best bid</strong> is the highest standing buy offer, so they are the prices a player can hit immediately. <strong>Mid</strong> is their midpoint and is a reference only; no one is obliged to trade there. <strong>Quoted spread</strong> is the gap between best bid and best ask as a percentage of mid, and it is a round-trip cost before the Market fee. <strong>Bid depth</strong> and <strong>ask depth</strong> count the TC resting on each side, so an order larger than the depth walks the book into worse prices.</p></details></div><div class="table-tools"><input id="worldSearch" type="search" placeholder="Search worlds…" aria-label="Search all worlds"></div></div><div class="table-wrap"><table class="data-table"><thead><tr><th>World</th><th>Best ask<span class="term-help">What you pay for 1 TC</span></th><th>Best bid<span class="term-help">What you receive for 1 TC</span></th><th>Mid<span class="term-help">Halfway point, not executable</span></th><th>Quoted spread<span class="term-help">Gap between the two prices</span></th><th>Bid depth<span class="term-help">TC buyers want</span></th><th>Ask depth<span class="term-help">TC offered for sale</span></th></tr></thead><tbody id="worldTable"></tbody></table></div></article>
      </section>
...
function isoDate(value){if(typeof value==="string"&&/^\d{4}-\d{2}-\d{2}/.test(value))return value.slice(0,10);const parsed=new Date(value);return Number.isNaN(parsed.valueOf())?String(value??""):parsed.toISOString().slice(0,10)}
function isoTimestamp(value){const parsed=value instanceof Date?value:new Date(value);if(Number.isNaN(parsed.valueOf()))return String(value??"");const stamp=parsed.toISOString();return`${stamp.slice(0,10)} ${stamp.slice(11,16)} UTC`}
...
function renderWorldChart(){
  ...
  const descriptions={
    mid:`Executed mid — the average of what players paid when buying and received when selling TC each day. Period: ${fixedWindow}. Days without data stay empty.`,
    buy:`Buy TC — what players paid, on average, to buy one TC each day. Period: ${fixedWindow}. Days without data stay empty.`,
    sell:`Sell TC — what players received, on average, for selling one TC each day. Period: ${fixedWindow}. Days without data stay empty.`,
    return:`Return — how much the executed mid rose or fell since the first available day. Period: ${fixedWindow}. 0% means no change.`
  };
...
function drawTwoSeries(svg,tooltip,rows,labels,mode){
  ...
  for(const index of uniqueIndexes(Math.min(width<600?3:6,rows.length),rows.length))addSvg(svg,"text",{x:x(index),y:height-13,"text-anchor":"middle",fill:"#647087","font-size":10},isoDate(rows[index].date));
  ...
  const cross=addSvg(svg,"line",{x1:m.left,x2:m.left,y1:m.top,y2:m.top+ih,stroke:"#34405a",opacity:0});
  const measureLabel={mid:"executed mid, the daily average price",buy:"the price paid to buy TC",sell:"the price received for selling TC",return:"return, the price change since the start"}[mode];
...
function renderWorldFacts(){
  ...
  $("#worldFacts").innerHTML=`<h2>${escapeHtml(name)}</h2>
    <div class="fact"><span><b>Best ask</b><small>what you pay to buy 1 TC now</small></span><strong>${book.best_ask?fmt.format(book.best_ask)+" GP":"No TC for sale"}</strong></div>
    <div class="fact"><span><b>Best bid</b><small>what you receive for selling 1 TC now</small></span><strong>${book.best_bid?fmt.format(book.best_bid)+" GP":"No buyer"}</strong></div>
    <div class="fact"><span><b>Mid</b><small>halfway between the two; a reference, not executable</small></span><strong>${book.mid?fmt.format(book.mid)+" GP":"—"}</strong></div>
    <div class="fact"><span><b>Quoted spread</b><small>gap between buying and selling, before the Market fee</small></span><strong>${book.quoted_spread_pct!==null&&book.quoted_spread_pct!==undefined?num(book.quoted_spread_pct).toFixed(1)+"%":"—"}</strong></div>
    <div class="fact"><span><b>Bid / ask depth</b><small>TC buyers want / TC offered for sale</small></span><strong>${book.bid_depth_tc?compact(book.bid_depth_tc)+" TC":"—"} / ${book.ask_depth_tc?compact(book.ask_depth_tc)+" TC":"—"}</strong></div>
    <div class="fact"><span><b>Order book snapshot</b><small>when this offer list was read</small></span><strong>${book.update_time?isoTimestamp(new Date(num(book.update_time)*1000)):"—"}</strong></div>
    <div class="fact"><span><b>Latest executed mid</b><small>average of what players paid and received that day</small></span><strong>${row.px_last?fmt.format(row.px_last)+" GP":"—"}</strong></div>
    ...
    <div class="fact"><span><b>Predicted 7d</b><small>model estimate of the change versus other worlds</small></span><strong class="${num(prediction?.predicted_change_pct)>=0?"positive":"negative"}">${prediction?signed(prediction.predicted_change_pct):"Not enough data"}</strong></div>`;
}
...
function renderWorldTable(){
  ...
  $("#worldTable").innerHTML=rows.map(row=>`<tr data-world="${escapeHtml(row.world)}" class="${row.world===$("#worldA").value?"selected":""}">
    <td data-label="World"><strong>${escapeHtml(row.world)}</strong></td>
    <td data-label="Best ask · what you pay">${orderBookMap.get(row.world)?.best_ask?fmt.format(orderBookMap.get(row.world).best_ask):"—"}</td>
    <td data-label="Best bid · what you receive">${orderBookMap.get(row.world)?.best_bid?fmt.format(orderBookMap.get(row.world).best_bid):"—"}</td>
    <td data-label="Mid · halfway point">${orderBookMap.get(row.world)?.mid?fmt.format(orderBookMap.get(row.world).mid):"—"}</td>
    <td data-label="Quoted spread · gap between the two prices">${orderBookMap.get(row.world)?.quoted_spread_pct!==null&&orderBookMap.get(row.world)?.quoted_spread_pct!==undefined?num(orderBookMap.get(row.world).quoted_spread_pct).toFixed(1)+"%":"—"}</td>
    <td data-label="Bid depth · TC buyers want">${orderBookMap.get(row.world)?.bid_depth_tc?compact(orderBookMap.get(row.world).bid_depth_tc)+" TC":"—"}</td>
    <td data-label="Ask depth · TC offered for sale">${orderBookMap.get(row.world)?.ask_depth_tc?compact(orderBookMap.get(row.world).ask_depth_tc)+" TC":"—"}</td></tr>`).join("");
